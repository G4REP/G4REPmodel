...
- `G4REPmodel.py`: the python code script that can be run to reproduce our model (importing the data, extracting the embeddings, training and testing).
- `Prediction.py`: the python code script that can be run to predict new **RG4BPs** (importing the data, extracting the embeddings and testing).
- `data`: the directory contains the training, test and validation datasets in the .fasta format.
- `examples`: the directory contains the human proteome in the .fasta format.
- `models`: the direcotry contains the model generated by our method.
